...
def main() -> None:
    ...
    def rt(kind: str) -> str:
        v = versions.get(kind, "")
        return {"rustc": "rustc", "ghc": "GHC", "ocaml": "OCaml", "lean": "Lean"}[kind] + (f" {v}" if v else "")
    ...
    block = f"""<!-- bench:begin -->
_Generated by bench/run-all.sh on {stamp} · {machine} · Lean {lean_v}{", " + others if others else ""} · [raw log](results/{stamp}/raw.txt)_
